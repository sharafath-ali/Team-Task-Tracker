...
import { getProjectMembers, addProjectMember, removeProjectMember, updateProjectMember } from '../api/projects.api';
...
export default function ProjectMembersPage() {
  ...
  const [addForm, setAddForm] = useState({ user_id: '', project_role: 'MEMBER' });
  ...
  const { data: membersData, isLoading } = useQuery({
    ...
    queryFn: () => getProjectMembers(selectedProject.id).then(r => r.data.data.members),
  });
  ...
  const addMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['project-members', selectedProject.id] });
  ...
  const updateRoleMut = useMutation({
    mutationFn: ({ userId, project_role }) => updateProjectMember(selectedProject.id, userId, { project_role }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['project-members', selectedProject.id] }),
  });
        ...
        {isLoading ? (
          ...
          <div className="empty-state">
            ...
            <p className="empty-state-desc">
              {isAdmin ? 'Add members to this project to collaborate.' : 'No members have been added to this project.'}
            </p>
          </div>
        ) : (
          <div className="members-grid">
            {members.map(m => (
              <div key={m.id} id={`member-${m.user_id}`} className="member-card">
                <div className="member-card-avatar">{getInitials(m.user_name)}</div>
                <div className="member-card-info">
                  ...
                  <div className="member-card-badges">
                    <span className={`user-role-badge role-${m.org_role}`}>{m.org_role}</span>
                    {isAdmin ? (
                      <select
                        className="role-select-inline"
                        value={m.project_role}
                        onChange={e => updateRoleMut.mutate({ userId: m.user_id, project_role: e.target.value })}
                      ...
                      <span className={`project-role-badge project-role-${m.project_role}`}>
                        {m.project_role}
                      </span>
                    )}
                  </div>
                ...
                {isAdmin && m.user_id !== user?.id && (
                  <button
                    ...
                    onClick={() =>
                      window.confirm(`Remove ${m.user_name} from ${selectedProject.name}?`) &&
      ...
      {showAdd && (
        ...
        >
          <div className="modal" style={{ maxWidth: 420 }}>
            ...
            <form onSubmit={e => { e.preventDefault(); setError(''); addMut.mutate(); }}>
              <div className="form-group">
                <label className="form-label" htmlFor="add-member-user">User *</label>
                <select
                  id="add-member-user"
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={e => setAddForm(f => ({ ...f, project_role: e.target.value }))}
                >
                  <option value="MEMBER">Member</option>
                  <option value="OWNER">Owner</option>
                </select>
