...
const listTasks = async ({
  ...
  if (effectiveAssignee)
    baseQuery.where("tasks.assignee_id", effectiveAssignee);
...
const getTaskById = async ({ taskId, orgId, userId, userRole }) => {
  const task = await TASK_JOINS(
    db("tasks").where({ "tasks.id": taskId, "tasks.org_id": orgId }),
  ).first();
...
const createTask = async ({
  ...
  const project = await db("projects")
    .where({ id: project_id, org_id: orgId })
    .first();
  if (!project)
    throw new AppError(
  ...
  if (assignee_id) {
    const assignee = await db("users")
      .where({ id: assignee_id, org_id: orgId, is_active: true })
      .first();
    if (!assignee)
      throw new AppError(
...
const updateTask = async ({ taskId, orgId, userId, userRole, updates }) => {
  ...
  if (updates.assignee_id && updates.assignee_id !== task.assignee_id) {
    const assignee = await db("users")
      .where({ id: updates.assignee_id, org_id: orgId, is_active: true })
      .first();
    if (!assignee)
      throw new AppError(
        "New assignee not found in your organization",
        404,
        "NOT_FOUND",
      );
  }
