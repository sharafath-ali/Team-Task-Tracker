...
const listProjects = async ({ orgId, userId, userRole, page = 1, limit = 20 }) => {
  ...
  if (userRole !== 'ADMIN') {
    ...
    baseQuery = baseQuery
      .join('project_members', function () {
        this.on('project_members.project_id', '=', 'projects.id')
  ...
  const projects = await baseQuery
    ...
    .leftJoin(
      db('project_members').select('project_id').count('* as member_count').groupBy('project_id').as('mc'),
      'mc.project_id', 'projects.id'
    )
    ...
    .leftJoin(
      db('tasks')
        ...
        .groupBy('project_id')
        .as('otc'),
      'otc.project_id', 'projects.id'
    )
    // Total task count sub-query
    .leftJoin(
      db('tasks').select('project_id').count('* as total_task_count').groupBy('project_id').as('ttc'),
      'ttc.project_id', 'projects.id'
    )
    // Latest task update for "recent activity"
    .leftJoin(
      db('tasks').select('project_id').max('updated_at as last_activity').groupBy('project_id').as('la'),
      'la.project_id', 'projects.id'
    )
    .select(
      'projects.*',
...
const getProjectById = async ({ projectId, orgId, userId, userRole }) => {
  let query = db('projects')
    ...
    .leftJoin(
      db('tasks').select('project_id').count('* as open_task_count').whereNotIn('status', ['DONE']).groupBy('project_id').as('otc'),
      'otc.project_id', 'projects.id'
    )
  ...
  if (userRole !== 'ADMIN') {
    ...
    if (!membership) throw new AppError('You are not a member of this project', 403, 'FORBIDDEN');
  }
...
const createProject = async ({ orgId, createdBy, name, description }) => {
  ...
  await db('project_members').insert({ project_id: project.id, user_id: createdBy, project_role: 'OWNER' });
