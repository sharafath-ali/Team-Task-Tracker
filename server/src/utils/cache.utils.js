...
const invalidateProjectCache = async (projectId) => {
  ...
  try {
    ...
    if (deleted > 0)
      logger.debug(
        `🗑️  Cache invalidated: ${deleted} key(s) for project ${projectId}`,
      );
  } catch (err) {
    logger.warn(
      `Cache invalidation failed for project ${projectId}: ${err.message}`,
    );
  }
};
...
const invalidateAssigneeCache = async (assigneeId) => {
  ...
  try {
    ...
    if (deleted > 0)
      logger.debug(
        `🗑️  Cache invalidated: ${deleted} key(s) for assignee ${assigneeId}`,
      );
  } catch (err) {
    logger.warn(
      `Cache invalidation failed for assignee ${assigneeId}: ${err.message}`,
    );
  }
};
