...
exports.seed = async (knex) => {
  ...
  const [admin] = await knex('users')
    .insert({ org_id: org.id, email: 'admin@demo.com', password_hash: hash, name: 'Admin User', role: 'ADMIN' })
    .returning('*');

  const [manager] = await knex('users')
    .insert({ org_id: org.id, email: 'manager@demo.com', password_hash: hash, name: 'Manager User', role: 'MANAGER' })
    .returning('*');

  const [member] = await knex('users')
    .insert({ org_id: org.id, email: 'member@demo.com', password_hash: hash, name: 'Member User', role: 'MEMBER' })
    .returning('*');
  ...
  await knex('tasks').insert([
    {
      org_id: org.id, project_id: project.id, created_by: admin.id,
      assignee_id: member.id, title: 'Set up CI/CD pipeline',
      description: 'Configure GitHub Actions for automated deploys',
      priority: 'HIGH', status: 'TODO',
      due_date: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000), // 5 days from now
    },
    {
      org_id: org.id, project_id: project.id, created_by: manager.id,
      assignee_id: member.id, title: 'Write API documentation',
    ...
    {
      org_id: org.id, project_id: project.id, created_by: admin.id,
      assignee_id: manager.id, title: 'Design database schema',
    ...
    {
      org_id: org.id, project_id: project.id, created_by: admin.id,
      assignee_id: admin.id, title: 'Project kickoff meeting',
